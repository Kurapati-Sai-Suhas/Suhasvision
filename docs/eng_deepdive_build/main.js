...
const doc = new Document({
  ...
  title: "SuhasVision — Engineering Deep-Dive & Onboarding Manual",
  numbering: {
    config: [
      {
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            ...
            children: [new TextRun({ text: "SuhasVision — Engineering Deep-Dive & Onboarding Manual", size: 15, color: COL_MUTE, font: FONT_BODY, italics: true })],
          })],
        }),
      },
      footers: {
...
Packer.toBuffer(doc).then((buf) => {
  fs.writeFileSync("./SuhasVision_Engineering_Deepdive.docx", buf);
  console.log("Wrote SuhasVision_Engineering_Deepdive.docx,", buf.length, "bytes");
});
